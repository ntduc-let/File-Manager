...
import com.ntduc.basemvvmarchitecture.view.viewholder.ItemMediaViewHolder

class MediaAdapter : BaseAdapter() {

  init {
    addSection(arrayListOf<Media>())
  }

  fun addMediaList(medias: List<Media>) {
    sections().first().run {
      clear()
      addAll(medias)
  ...
  override fun layout(sectionRow: SectionRow) = R.layout.item_media

  override fun viewHolder(layout: Int, view: View) = ItemMediaViewHolder(view)
}
